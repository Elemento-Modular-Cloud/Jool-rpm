...
Release:        1%{?dist}
Summary:        Jool NAT64/MAP-T kernel module (DKMS)
License:        GPL-2.0
BuildArch:      noarch
...
%post
dkms add     -m jool -v %{version} --rpm_safe_upgrade
dkms build   -m jool -v %{version}
dkms install -m jool -v %{version}

%preun
dkms remove -m jool -v %{version} --all --rpm_safe_upgrade || true
